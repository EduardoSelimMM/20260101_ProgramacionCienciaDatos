...
La sesión pasada ya vimosl algunos tipos atómicos de datos.

Para saber de qué tipo atómico es un objeto, ocupamos la función `class()`
...
```{r}
# f o g o h o j (x)
# la o es bolita de composición matemática
3.6 |> sin() |> abs() |> sqrt() |> tan()
```

+ También dijimos que las estructuras básicas eran: vectoes, matrices, listas, dataframes

## Vectores

+ Para crear un vector, usamos la función `c()` de concatenar
...
+ por supuesto, puedo sumar dos vectores numéricos

```{r}
mi_vector1 + mi_vector2
```

+ ¿Qué pasa si la longitud de los vectores no coincide?

```{r}
c(1,2,3) + c(4,5)
```
+ Lo hace, pero ojo está mal

```{r}
# Pensaríamos que este código nos mandaría un error pero NO
c(1,3,5,8,9,10) + c(4,5)
```
1 + 4 = 5
3 +5 = 8
5 + 4 = 9
...
Si la longitud del vector grande es múltiplo de la longitud del vector chico, copia tantas veces como quepa, el vector chico en el vector grande

+ Mi opinión: Más que ayudar, esto perjudica

```{r}
# Si a fuerza quiero hacer la misma operación
c(1,3,5,8,9,10) + c(4,5,4,5,4,5)
```

+ Por supuesto, podemos crear vectores a partir de otros ya existentes
...
+ Dijimos que los vectores en R están diseñados para guardar objetos de la misma clase atómica. Si cometemos el pecado de guardar objetos de diferente clase, el objeto 'character' tiene prioridad

```{r}
vector_mounstro <- c(4, "Hola mundo", TRUE, pi)
vector_mounstro
```
+ Sí lo guardó, pero cada entrada la convirtió en "string"

```{r}
vectorcillo <- c(1,3,4,FALSE,TRUE)
vectorcillo
```
+ TRUE: 1
+ FALSE: 0
...
```{r}
# El número de entradas del vector
length(simpsons)
```
+ Para acceder a las entradas de un vector ocupamos los corchetes cuadrados [ ]

```{r}
# La tercera entrada del vector
simpsons[3]
```

```{r}
# Los elementos de las entradas 2 hasta la 4 incluyéndolas
simpsons[2:4]
```
```{r}
# Si quiero varias posiciones específicas
simpsons[c(1,3,4)]
```

+ Una característica cool de R, es que le podemos poner nombre a cada una de las entradas de un vector. Para hacer eso, usamos la función `names()`
...
+ ¿Porqué está cool tener nombres?

Para poder referirnos a las entradas del vector por nombre y no por posición
...
```{r}
simpsons[c('papá', 'hijo')]
```

+ La cosa se "descompone" cuando le agregas una entrada al vector, pues esta nueva entrada no tendrá nombre

```{r}
simpsons_con_vecino <- c(simpsons,"Flanders")
simpsons_con_vecino
```
+ ¿Cómo lo arreglamos? "A mano"

```{r}
names(simpsons_con_vecino)[6] <- "vecino"
```
...
## Algunas estructuras númericas nativas de R

```{r}
# Crea un vector del 1 a 30
x <- 1:30
x
```
...
y
```
+ La operación `:` tiene prioridad. Primero crea el vector

```{r}
1:10-1
```
Primera crea el vector del 1 al 10 y luego le resta 1 a cada entrada.

A mí me gusta más, usar paréntesis

```{r}
(1:10) -1
```
+ Los parétesis ayudan un montón a leer nuestro propio código (en el futuro)
...
```{r}
# Crea un vector del 1 al 5 con 9 entradas
...
```{r}
# Vector que empieza en 1 y avanza de 0.5 en 0.5 hasta acercarse lo más posible al 5
seq(1, 5, 0.5)
```
```{r}
# Vector que empieza en 1 y avanza de 0.3 en 0.5 hasta acercarse lo más posible al 5
seq(1, 5, 0.3)
```
Esta sintaxis la odio!! No somos cavernícolas. Me gusta sintaxis explícita (no posicional)

```{r}
# Sintaxis de cavernícola
seq(1, 5, 0.5)
```
```{r}
# Sintaxis de persona
...
+ Mejor sintaxis y comentarios es una carta de amor a nosotros mismos del futuro.

```{r}
# Repite la primera entrada el número de veces que dice la segunda entrada
rep(1, 30)
```
...
```{r}
# Repite el vector 1,3,8 siete veces
rep(x = c(1,3,8), times = 7)
```

```{r}
# Puede confundirse con el seq
sequence(4:8)
```
...
sequence(5)
```
Me devuelve del 1 al 5

+ La función sequence está vectorizada. Significa que le puedo meter un vector y aplicar la operación entrada a entrada
...
sequence(c(3,7,2))
```
Ahora sí ya es más natural entender

```{r}
# 4:8 es equivalente a c(4,5,6,7,8)
sequence(4:8)
```

+ Podríamos cambiar el valor de alguna entrada

```{r}
mi_vector <- c("Hola", "Mundo", "Programación", "Vurro")
mi_vector
```
+ Me gustaría cambiar la entrada 4

```{r}
mi_vector[4] <- "burrito sabanero"
mi_vector
```
+ Si quisiera eliminar una entrada
...
```{r}
mi_vector <- mi_vector[1:3]
mi_vector
```

```{r}
# Quito lo que está en la posición 2
mi_vector[-2]
```
...
```{r}
otro_vector <- c(2,8,7,45)
# otro_vecto[ultima entrada] = otro_vector[4]
otro_vector[length(otro_vector)]
```
...
identidad |> class()
```
Otra forma es con la función `str()` 'structure':
...
```{r}
matrix(data = c(2,6,7,8,3), nr = 2, nc = 2)
```
```{r}
matrix(data = c(2,6,7,8,3), nr = 2, nc = 3)
```
```{r}
matrix(data = c(2,6,7,8,3), nr = 3, nc = 3)
```
...
[i,j]: i es el reglón y j es la columna
